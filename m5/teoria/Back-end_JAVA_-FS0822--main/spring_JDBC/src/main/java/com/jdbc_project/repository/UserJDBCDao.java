...
public class UserJDBCDao implements UserDaoRepository {
	...
	public void create(User user) {
		String sql = "INSERT INTO public.user (name, lastname, email, age, city, password) "
				   + "VALUES (?, ?, ?, ?, ?, ?)";
		jdbc.update(sql, 
					user.getName(),
					user.getLastname(),
					...
					user.getCity(),
					user.getPassword());
		log.info("Utente " + user.getName() + " " + user.getLastname() + " salvato nel DB!!!");
	}
	...
	public void update(User user) {
		...
		jdbc.update(sql, 
				...
				user.getPassword(),
				user.getId());
		log.info("Utente " + user.getName() + " " + user.getLastname() + " modificato nel DB!!!");
	}
